...
img, cov, bold, nh = next(iter(loader))
img = img.to(device).float(); cov = cov.to(device); bold = bold.to(device); nh = nh.to(device)
model = InkUNet(base=16, levels=4).to(device)
...
def step(do_degrade=True):
    ...
    with torch.autocast("cuda", dtype=torch.float16):
        loss, _, _ = ink_losses(model(di), cov, bold, 1.5, strip_w=w)
